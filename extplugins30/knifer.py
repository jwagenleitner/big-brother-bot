    ...
    def updateHallOfFame(self, cutKillers, mapName):
        ...
        if (currentRecordHolder != '') and (currentRecordValue != '0'):
            ...
            if topKills > int(currentRecordValue):
                ...
                q = 'UPDATE %s SET %s=\'%s\', %s=\'%s\' WHERE %s=\'%s\'' % (self._db_tableHOF, self._db_playerid, str(bestPlayer.id), self._db_score, str(topKills), self._db_mapName, currentMap)
            ...
            currentRecordHolder = bestPlayer.exactName
            currentRecordValue = topKills
            q = 'INSERT INTO %s (%s, %s, %s) VALUES(\'%s\', %s, \'%s\')' % (self._db_tableHOF, self._db_mapName, self._db_playerid, self._db_score, currentMap, str(bestPlayer.id), topKills)
