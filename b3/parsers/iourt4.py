...
class Iourt4Client(Client):
    ...
    def auth_by_pbid(self):
        ...
        if len(clients_matching_pbid) > 1:
            ...
            try:
                client_by_guid = self.console.storage.getClient(Iourt4Client(guid=self.guid))
            except KeyError:
                pass
            else:
    ...
    def __str__(self):
        return "Client4<@%s:%s|%s:\"%s\":%s>" % (self.id, self.guid, self.pbid, self.name, self.cid)
...
class Iourt4Parser(AbstractParser):
    ...
    def pluginsStarted(self):
        ...
        """
        # TODO: from 4.1, should this be removed since 4.2 didn't call this?
    ...
    def patch_Clients():
        ...
        def newClient(self, cid, **kwargs):
            ...
            """
            client = Iourt4Client(console=self.console, cid=cid, timeAdd=self.console.time(), **kwargs)
